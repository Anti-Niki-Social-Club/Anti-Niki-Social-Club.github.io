...
// Set image sources
spriteImages.car.src = 'public/car.png';
spriteImages.truck.src = 'public/car.png'; // You can replace with truck.png if available
spriteImages.log.src = 'public/car.png'; // You can replace with log.png if available
spriteImages.turtle.src = 'public/car.png'; // You can replace with turtle.png if available
spriteImages.frog.src = 'public/car.png'; // You can replace with frog.png if available

// Image loading tracker
let imagesLoaded = 0;
const totalImages = Object.keys(spriteImages).length;

// Track when all images are loaded
Object.values(spriteImages).forEach(img => {
  img.onload = () => {
    imagesLoaded++;
    if (imagesLoaded === totalImages) {
      console.log('All sprite images loaded');
    }
  };
  img.onerror = () => {
...
Sprite.prototype.render = function () {
  ...
  // Determine the appropriate sprite image
  let spriteImage = null;

  // Select appropriate image based on sprite type
  if (this.spriteType && spriteImages[this.spriteType] && spriteImages[this.spriteType].complete) {
    spriteImage = spriteImages[this.spriteType];
  }

  // draw a rectangle sprite
  if (this.shape === 'rect') {
    if (spriteImage) {
      // Draw image for rectangle sprites
      context.drawImage(
        spriteImage,
        this.x,
        this.y + gridGap / 2,
...
const frogger = new Sprite({
  ...
  size: grid,
  shape: 'circle',
  spriteType: 'frog' // Add sprite type
});
